...
const setMPA = () => {
  ...
  const entries = glob.sync(path.join(projectRoot, './src/*/index.js'));
  ...
  entries.map((filePath) => {
    ...
    return htmlWebpackPlugins.push(
      new HtmlWebpackPlugin({
        template: path.join(projectRoot, `./src/${pageName}/index.html`), // 模版所在位置
        filename: `${pageName}.html`, // 指定打包出的文件名称
        chunks: ['vendors', pageName], // html使用哪些chunk
